...
//    LCD_DisplayStringLine(Line6, (uint8_t*)"      failed      ");
  ...
  Eth_Link_PHYITConfig(LAN8720_PHY_ADRESS);
...
static void ETH_SetLED()
{
    ...
    ETH_WritePHYRegister(LAN8720_PHY_ADRESS, 0x18, 0x0000);


    reg_value = ETH_ReadPHYRegister(LAN8720_PHY_ADRESS, 0x19);
    reg_value &= ~0x00E0;
    reg_value |= 0x00C0;
    ETH_WritePHYRegister(LAN8720_PHY_ADRESS, 0x19, reg_value);
}
...
static void ETH_MACDMA_Config(void)
{
  ...
  /* Configure Ethernet */
  EthInitStatus = ETH_Init(&ETH_InitStructure, LAN8720_PHY_ADRESS);
}
